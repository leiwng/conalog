...
router.get("/", (req, res, next) => {
  ...
  if (id !== undefined) query["_id"] = new ObjectID(id);
  if (category !== undefined) query["category"] = category;

  mongoProvider
    .list(constants.COLLECTOR_COLL, query, constants.COLLECTOR_LIMIT, null, 0)
    .then((collectors) => {
      res.json(collectors);
    })
    .catch((error) => {
...
router.get("/:id", (req, res, next) => {
  ...
  mongoProvider
    .query(constants.COLLECTOR_COLL, query)
    .then((collector) => {
      res.json(collector);
    }) // query
    .catch((error) => {
...
router.post("/", function (req, res, next) {
  ...
  mongoProvider
    .insert(constants.COLLECTOR_COLL, collector)
    .then((result) => {
      if (result.result.n > 0) {
        ...
        logger.error(
          ...
          "Collector insert failed, collector: " + JSON.stringify(collector)
        );
        res.sendStatus(500);
      }
    }) // mongoProvier.insert
...
router.put("/", function (req, res, next) {
  ...
  mongoProvider
    .update(constants.COLLECTOR_COLL, query, collector)
    .then((result) => {
      if (result.result.ok > 0) {
        ...
        logger.error(
          ...
          "Collector update failed, collector: " + JSON.stringify(collector)
        );
...
router.delete("/:id", (req, res, next) => {
  ...
  if (id === undefined || id === null) res.sendStatus(412);

  let query = { _id: new ObjectID(id) };
  mongoProvider
    .delete(constants.COLLECTOR_COLL, query)
    .then((result) => {
      if (result.result.ok > 0) {
    ...
    .catch((err) => {
      logger.error(
        ...
        null,
        "Collector Delete Problem",
        "Collector delete failed, error: " + err.stack
      );
    ...
    mongoProvider
      ...
      .then(function (result) {
        if (result.result.n > 0) {
          ...
          logger.warning(
            ...
            null,
            "Collector Delete Problem",
            "Collector delete failed, id: " + list
          );
          res.sendStatus(200);
        }
      }) // delete
      .catch(function (err) {
        logger.error(
          ...
          null,
          "Collector Delete Problem",
          "Collector delete failed, error: " + JSON.stringify(error)
        );
        res.sendStatus(500);
      });
  } else {
      ...
      if (list !== undefined && list !== null) {
        ...
        let statusList = list
          .map((collector) => {
            ...
            if (statusJson.status.runningFlag) return statusJson;
            else return null;
          })
          .reduce((prev, curr, index) => {
            if (curr !== null) return prev.push(curr);
            else return prev;
          }, []);
